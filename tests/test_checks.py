...
def test_dm_check_for_guild(context):
    context.message.guild_id = 3123
    assert checks.dm_check(context) is False

def test_guild_check(context):
    with mock.patch.object(checks, "dm_check", return_value=False):
        assert checks.guild_check(context) is True
        checks.dm_check.assert_called_once_with(context)
